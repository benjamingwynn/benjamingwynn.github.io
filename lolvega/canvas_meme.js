/*	fork of particlebox 3.1
		(C) Benjamin Gwynn (Xenxier), 2015
		http://xenxier.com
		
	i know guys this abuses the global scope etc, i was learning javascript when i wrote this
*/
...
//				context.fillRect(particlebox.stack[z][i]["x_position"], particlebox.stack[z][i]["y_position"], particlebox.stack[z][i]["x_size"], particlebox.stack[z][i]["y_size"]);
				context.drawImage(image, particlebox.stack[z][i]["x_position"], particlebox.stack[z][i]["y_position"], particlebox.stack[z][i]["x_size"], particlebox.stack[z][i]["y_size"]);
			}
		}
	}
}
...
particlebox.control('draw_amount', 50);

particlebox.control('min_x_velocity', -1);
particlebox.control('max_x_velocity', 50);

particlebox.control('min_y_velocity', 10);
particlebox.control('max_y_velocity', -40);

particlebox.control('z_depth', 3);
particlebox.control('size_multiplier', 40);
...
//particlebox.control('bg_r', 0);
//particlebox.control('bg_g', 0);
//particlebox.control('bg_b', 0);

particlebox.create();

// use request animation frame because we live in the future
function draw () {
	particlebox.draw()
